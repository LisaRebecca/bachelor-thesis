...
\end{lstlisting}
	 ...
	 The code example \ref{code:cleaned_descr} shows the input and output of this procedure. A token is completely discarded if it contains non-alphabetical characters, as these tokens are more than often not an accurate descriptor of the text's meaning.	
        ...
        The majority of popular \ac{ML} algorithms require the input of scalar, vector or matrix data. Several methods for representing text as mathematical object will be discussed in the following.
           ...
           The model is trained on two tasks. Firstly, it is trained on word prediction. \ac{BERT} had to predict one missing word in a text unit. The objective of course is to get the right word. The loss in this task is measuring if and how wrong a prediction is. The second task is to predict the next sentence after a text section. The objective is to predict the correct words and their order. The model is trained by adjusting internal weights in such a way, that both losses are minimized. In turn this means, the model can predict missing words and next sentences with minimized error.
           
           In the following, each component of the model is explained.
            
            \subparagraph{\ac{MLM}} \ac{BERT} uses a \acl{MLM} for training. The model has the basic task of predicting a missing word in a sequence of words. This method is able to transform an unsupervised task into a supervised one. It does so by blacking out a part of the input text. The model then is tasked with predicting the missing parts. If the model did not predict the missing part correctly, the model will be adapted using gradient changes \cite{MLM}. 
             
             \begin{figure}[h!]
             	\label{fig:transformer}
             	\centering
             	\includegraphics[height=10cm]{Bilder/preprocessing/BERT/transformer_architecture.png}
             	\caption{The Transformer Architecture  \cite{transformersAttention}}
             \end{figure}
         
                      
               \subparagraph{Transformer}
               Fundamentally, BERT is powered by a transformer (\ref{fig:transformer}). A transformer describes a fundamental architecture for learning word representations \cite{transformersAttention}.
               BERT is trained with millions of wikipedia articles, meaning the inputs is a large text database. Each text segment is fed into the transformer. The text segment is split into tokens, which are then encoded as a vector of length 768 \cite{BERTEmbeddingLayer}. Next follows the positional encoding. When applying the transformer to a sequence of words, the output should be dependent of the sequence of the input words. E.g. the sentences "Bob is taller than Alice" and "Alice is taller than Bob" should be encoded as two different vectors. This can be realized with encoding the position of each token. 
               
               The token embedding and its positional encoding are now added, and fed into the encoder \cite{illustratedTransformer}. The encoder makes up the left half on an transformer. A transformer can consist of several sequential encoders, but requires at least one. Inside the encoder, four elements process the input sequentially:
               ...
               \end{enumerate}
       			...
       			The decoder is the right part of the transformer architecture. It has several components in common with the encoder.
            ...
            The advantage of bert over count-based
            
            
            The advantage of bert over word2vec
            
            
            \cite{evaluationTopicModelling}
            ...
            From the available pre-trained modles, the \lstinline|distiluse-base-multilingual-cased| is 
